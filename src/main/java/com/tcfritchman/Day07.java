...
public class Day07 {
    ...
    private static Object part1(String input) {
        return extracted(input, Set.of(Operator.ADD, Operator.MULTIPLY));
    }

    private static Object part2(String input) {
        return extracted(input, Set.of(Operator.ADD, Operator.MULTIPLY, Operator.CONCAT));
    }

    private static long extracted(String input, Set<Operator> operators) {
        long total = 0;
        for (String line : input.split("\n")) {
            long value = Long.parseLong(line.split(": ")[0]);
            List<Long> numbers = Arrays.stream(line.split(": ")[1].split(" ")).map(Long::parseLong).toList();

            List<Equation> equations = createExpressionTree(numbers, operators)
                    .stream()
    ...
    private static List<Expression> createExpressionTree(List<Long> numbers, Set<Operator> operators) {
        long n = numbers.getFirst();
        return createExpressionTreeInner(numbers.subList(1, numbers.size()), new ConstantExpression(n), operators);
    }

    private static List<Expression> createExpressionTreeInner(List<Long> numbers, Expression ex, Set<Operator> operators) {
        if (numbers.isEmpty()) {
            return List.of(ex);
        }
        long n = numbers.getFirst();
        ...
        for (Operator operator : operators) {
            List<Expression> exprs = createExpressionTreeInner(numbers.subList(1, numbers.size()), new BinaryExpression(ex, new ConstantExpression(n), operator), operators);
            l.addAll(exprs);
        }
        return l;
    }
    ...
    record ConstantExpression(long val) implements Expression {
        @Override
        public long evaluate() {
            return val;
        }
    ...
    static class Equation {
        ...
        public Equation(long value, Expression expression) {
            this.value = value;
